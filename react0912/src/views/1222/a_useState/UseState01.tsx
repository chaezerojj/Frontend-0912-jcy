...
// >> : 해당 상태의 변겅이 컴포넌트의 재렌더링을 유발
// : React가 렌더링을 다시 시작히여 DOM을 업데이트

//? 상태 변경의 비동기적 특성
// : useState로 상태를 변경하는 작업은 비동기적으로 이루어짐.
// : 두 번째 인자인 set설정 함수를 호출한 직후에 상태값이 즉시 변경되지 않을 수도 있다.
...
export default function UseState01() {
  ...
  const handleClick = () => {
    ...
    // : React는 성능 최적화를 위해 여러 상태의 변경을 한꺼번에 처리, 하나의 렌더링 사이클에서 일어날 수 있도록 설정
    console.log(count);
    
    // 이전 상태 값을 기반으로 상태를 업데이트 할 때에는 함수형 업데이트를 사용하는 것을 권장
