...
export default function Axios01() {
  ...
  // then: 비동기 작업이 완료되면 해당 url로부터 가져온 값을 받는 메소드
  // catch: 비동기 작업 또는 then에서 발생하는 에러를 받는 메소드

  // get(경로): 데이터를 서버로부터 가져올 때 사용
  ...
  axios.post('http://example.com/data', { name: 'New Items' })
  .then(response => {
    console.log(response.data);
  })
  .catch(error => {
  ...
  // axios.put('http://example.com/users/1', { name: 'Updated Items' })
