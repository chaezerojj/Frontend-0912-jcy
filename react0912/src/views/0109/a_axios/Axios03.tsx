...
// 새 게시물 생성 또는 기존 게시물 수정 함수
  const savePost = async () => {
    // 입력창에 title 또는 content 영역이 비워진 경우 동작 X
    if(!newPost.title || !newPost.content) return;

    try {
      // 수정중인 게시물의 id X: 생성 단계
      ...
      <List>
        {posts.map(post => (
          ...
          <ListItem key={post.id}>
            ...
            <Button onClick={() => handleDeletePost(post.id)} color='secondary'>Delete</Button>
          </ListItem>
