...
export default function HookReview() {
  ...
  // 컴포넌트가 마운트될 때 외부 API에서 데이터 불러오기
  useEffect(() => {
    // API 호출
    // fetch로 가져오는 응답을 JSON 형태로 반환
    // 처음 150개의 사진만 가져와서 상태에 저장
  ...
  const addPhoto = () => {
    ...
    const newPhoto: Photo = {
      // Math.max 함수: 해당 목록에서 가장 큰 숫자를 불러옴
      // map 함수를 사용하여 photos 배열의 각 사진에서 ID만 추출
      // , photos 배열이 비어있을 경우 기본값으로 0을 사용
      id: Math.max(...photos.map((photo) => photo.id)) + 1,
  ...
  const startEdit = (photo: Photo) => {
    ...
    // 수정중인 사진 제목을 입력 필드에 설정
    setEditTitle(photo.title);
  };

  // 사진 수정을 완료하는 함수
  const editPhoto = () => {
    // 사진 수정 완료시 렌더링된 사진 배열의 요소를 변경
    setPhotos(
      photos.map((photo) =>
        // ID가 일치하면 제목을 수정
