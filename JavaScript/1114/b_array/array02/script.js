...
// 문자열데이터.toUpperCase()
console.log('---------map---------');
let carsInUpperCase = cars.map(car =>
  car.toUpperCase());
console.log(carsInUpperCase); // ['AUDI', 'BMW', 'HYUNDAI', 'VOLVO']

// * filter: 특정 조건을 만족하는 요소만 추출하여 새 배열을 생성
...
// 배열명.reduce((accumulator, currentValue, currentIndex, array) => {
  //  함수 로직
// }, initialValue);
// accumulator: 이전 리듀서의 호출의 반환값
// : 첫번째 호출에서는 initialValue이 지정
// : 각 호출에서 축적되는 값
// currentValue: 현재 처리 중인 배열의 요소

// currentIndex: (선택)현재 처리중인 배열 요소의 인덱스
// array: (선택) reduce가 호출된 배열
// initialValue: (선택) 작성을 하지 않을 경우 배열의 첫번째 요소가 초기값으로 지정
...
// 데이터 사이에는 ', '
// let cars = ['audi', 'bmw', 'hyundai', 'volvo'];
// 출력값 audi, bmw, hyundai, volvo
let combinedCarNames = cars.reduce((accumulator, currentValue) => 
  accumulator + ', ' + currentValue
);
console.log(combinedCarNames); // audi, bmw, hyundai, volvo

//! 배열 요소의 정렬과 순서 변경
// 1. sort(compareFunction)
// : 배열 요소를 정렬
// compareFunction
// - 값이 생략될 경우, 배열의 요소들은 문자열로 취급, 유니코드값 순서대로 정렬
// 해당 함수가 리턴하는 값이 
...
// 두 숫자 a, b의 차(a-b)가 양수값인 경우 : b가 우선
// 움수값인 경우 : a가 우선
numbers = [3, 1, 4, 2, 5, 9];
...
let numberLocalString = numbers.toLocaleString();
console.log(typeof numberString); // string
console.log(numberLocalString); // 5,4,3,6,1