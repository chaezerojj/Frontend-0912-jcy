...
let mixedTypes = [1, 'two', false, [5, 6]]; // 다양한 데이터 삽입
...
// 배열의 요소 수정(인덱싱)
fruits[1] = 'mango';
console.log(fruits[1]); // mango (banana에서 mango로 수정)
...
// : 배열의 끝에 요소를 추가, 제거
let cars = ['bmw', 'audi'];
cars.push('hyundai'); // 배열 끝에 요소 추가
console.log(cars); // ['bmw', 'audi', 'hyundai']

//* 요소 추가
cars[7] = 'kia';
console.log(cars); // ['bmw', 'audi', 'hyundai', 비어 있음 × 4, 'kia']

// length속성을 사용하여 배열의 마지막 위치에 요소 추가
cars[cars.length] = 'toyota';
console.log(cars); // ['bmw', 'audi', 'hyundai', 비어 있음 × 4, 'kia', 'toyota']
...
let carsShift = cars.shift(); // 첫번째 요소 제거
console.log(carsShift); // bmw
console.log(cars); // ['audi', 'hyundai', 비어 있음 × 4, 'kia']
cars.unshift('newBMW'); // 시작에 추가
console.log(cars); // ['newBMW', 'audi', 'hyundai', 비어 있음 × 4, 'kia']
...
// : 생략 시에는 삽입되지 X
console.log(cars); // ['newBMW', 'audi', 'hyundai', 비어 있음 × 4, 'kia']
// 인덱스 3번부터 4개의 요소를 제거하고, 'jeep','volvo'를 삽입
let removedElements = cars.splice(3, 4, 'jeep', 'volvo');
console.log(removedElements); // [비어 있음 × 4]
...
// end(선택)
// : 생략 시에는 배열의 끝까지 추출
// : end번호 바로 앞의 인덱스까지만 추출
let slicedCars = cars.slice(1, 4); // 인덱스번호 1, 2, 3이 추출
console.log(slicedCars); // ['audi', 'hyundai', 'jeep']
console.log(cars); // ['newBMW', 'audi', 'hyundai', 'jeep', 'volvo', 'kia'] >> 요소 6개의 배열이 변함없이 추출됨
...
console.log(combinedCars); // ['newBMW', 'audi', 'hyundai', 'jeep', 'volvo', 'kia', 'tayo', 'tomas']

// join메소드 안에 문자열 값을 기준으로 배열의 요소들이 문자열로 반환
let carString = combinedCars.join(' ');
console.log(carString); // newBMW audi hyundai jeep volvo kia tayo tomas

console.log('-------split()-------');

//? 5. split() 메소드 (문자열의 메소드)
// 문자열로 이루어진 데이터를 정제하여
// , 배열의 형태로 나타낼 때 사용
let myData = 'Seoul, Busan, Daejeon';
let myArray = myData.split(', ');
console.log(myArray); // ['Seoul', 'Busan', 'Daejeon']