...
// : 코드의 재사용성 & 가독성 향상, 그리고 디버깅을 용이하게 함
...
//! 5. 함수의 매개변수
// : 함수를 정의하고 호출하는 과정에서 함수에 데이터 전달방식
...
//? 인수 VS 파라미터
// 함수 정의
function sum(a,b) { // 소괄호 안의 a,b는 피라미터
  return a + b;
}
...
// 함수 호출
hello('Doraemi'); // 'Doraemi'는 인수

//! 6. 자바스크립트의 파라미터와 인수 사용
// : 함수 호출 시 전달되는 인수의 수와
// : 함수 정의 시 선언된 파라미터의 수가 일치하지 않아도 가능

function log(a) { // 한개의 파라미터
  console.log(a);
}

log(); // 인수를 넘기지 X - 출력: undefined
log(1); // 인수를 하나 넘김 - 출력: 1
log(1,2); // 인수를 두개 이상 넘김 - 출력: 1 (지정된 수의 파라미터 이상의 인수는 읽히지 X)
...
let seungAh = 'SeungAh' // 전역변수
function lsa() {
  let job = ' Developer' // 지역변수
  console.log(job);
  return seungAh  + job;
}

console.log(lsa());
// console.log(job); - Error (job이 정의되지 않아 함수 바깥으로 나오면 오류)