document.addEventListener("DOMContentLoaded", () => {
        ...
        <div class="card">
          <div class="card-inner">
            <div class="card-front">
              <img src="./images/puppy-2.jpg" alt="카드의 앞면" />
            </div>
  ...
  function revealCardTemporarily() {
    // 'completed-button'  버튼 비활성화
    // : 카드가 공개되어 있는 동안 클릭 X
    ...
    // 2000밀리초(2) 후에 모든 카드를 다시 뒤집어 원래 상태로 돌림
    setTimeout(() => {
  ...
  // 카드가 뒤집혔는지 여부, 첫번째와 두번째 선택된 카드, 게임판이 잠겨있는지 여부를 나타내는 변수
  ...
  function flipCard() {
    ...
    if (!hasFlippedCard) {
      ...
      // 두번째 카드를 뒤집은 경우
      // 다음 선택을 위해 'hasFlippedCard'를 false설정
  ...
  startButton.addEventListener('click', () => {
    ...
    // 카드를 잠시동안 공개하는 함수
    revealCardTemporarily();
  });

  //! 'reset-button'버튼을 초기화
  ...
  completedButton.addEventListener('click', () => {
    ...
    // Array.from()는 NodeList를 배열로 반환
    // every함수는 배열의 모든 요소가 주어진 함수를 만족할 때 true 반환
    const allFlipped = Array.from(document.querySelectorAll('.card'));
    querySelectorAll('.card').every(card => {
...
function shuffle() {
  // 배열의 마지막 요소부터 시작하여 첫번째 요소까지 역순으로 반복
