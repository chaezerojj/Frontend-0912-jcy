...
//? 유니언 타입
// 사용자가 string, number 또는 boolean 타입의 데이터를 입력
...
function processInput(input: string | number | boolean): string | number | boolean {
  // 유니언 타입 사용시 타입 제한을 하지 않은 경우에는
  // 유니언 타입으로 지정된 모든 타입에서 중복되는 속성만! 사용 가능
  // input.toFixed(); -> Error 해당 속성은 string에서 사용할 수 없기 때문에 오류
  // >> 유니언 타입은 교집합
