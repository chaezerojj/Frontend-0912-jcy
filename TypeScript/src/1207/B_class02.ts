...
// : 반환값X, 자신을 new 키워드와 함께 호출하는 대상(클래스)의 새로운 인스턴스를 암시적으로 변환
...
let person1 = new Person('jcy', 25);
console.log(person1.describe());
console.log(person1.name);
console.log(person1.age);

let person2 = new Person('ldk', 28);
...
// 속성 기본값
// : 함수의 기본 인자와 유사한 개념으로 클래스 속성에도 기본값이 제공 가능
...
// triangle.height = 5; - 읽기 전용 속성이므로 외부에서 값 할당 시 오류 생성
...
class Human {
  ...
  constructor() {
    ...
    setInterval(() => {
      ...
      // 화살표 함수의 this 값은 함수가 호출되는 방식과 무관하게, 함수가 작성된 위치에 따라 결정
      // : 객체 내에 화살표 함수를 정의하면
...
console.log(myObject.regularFuction());
