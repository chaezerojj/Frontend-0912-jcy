{
//! 제네릭, 클래스, 배열 메소드를 이용한 회원 관리 프로그램 구현
...
//?  회원관리 프로그램
// : 회원을 추가(addMember), 검색(findMemder), 제거(removeMember), 활성회원만 필터링(filterActiveMembers)하는 기능을 제공
...
membership.addMember(new Member('ldk', 35, false));
membership.addMember(new Member('ljg', 29));

// 'ldk'라는 이름을 가진 회원을 출력
console.log(membership.findMember('ldk')); // Member { name: 'ldk', age: 35, active: false }
