...
// : , 해당 컴포넌트를 사용하는 시점에서 원하는 데이터 타입을 지정 가능
...
//* 여러 제네릭 타입 변수 지정
// : 여러개의 독립적인 타입을 처리해야 할 때 여러 타입 변수를 활용
...
let resultFunction = genericFunc<boolean>([true, false, true]); // 3
console.log(resultFunction); // [ true, false, true ]
...
let myGenericvar = new GenericClass<number>(0, function(x, y) {
  return x + y;
});
let myGenericStringVar = new GenericClass<string>('', function(x, y) {
  return `${x}, ${y}`;
})
console.log(myGenericvar.add(5, 3)); // 8
console.log(myGenericStringVar.add('Hello', 'hi')); // Hello, hi
